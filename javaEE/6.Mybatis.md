...
- 工作原理：先根据Mapper接口全类名+方法名 作为key值，用于快速定位是哪个映射文件的哪个方法中的哪个标签。底层通过jdk动态代理方式 运行时会给每一个Mapper接口创建一个Proxy(代理对象) 会拦截Mapper接口方法，拦截之后 帮你动态的调用哪个映射文件的哪个标签 就可以执行sql语句，最终执行结束后 运行结果 也会通过拦截器 按照你方法提供的返回值 进行封装
