...
> 工作原理：服务器启动加载web.xml，通过配置好的过滤器全类名 实现反射来实例化过滤器 调用init() 做初始化，过滤器可以配置多个 就会形成一个过滤器栈 遵循先进后出的原则，过滤器的执行顺序是通过filter-mapping先后顺序决定的。每个过滤器必须通过doFilter()进行连接 最后只有经过这一个个过滤器才能达到最后访问的资源
